...
class decoder(nn.Module):
  ...
  def forward(self, images, input_ids = None):
    ...
    with torch.no_grad():
      batch_features = self.__cnn__(images)
    
    conv_features = self.__img2embed_conv__(batch_features).reshape(batch, int(self.embed_dim * 0.5), self.block_num ** 2).permute(0, 2, 1)
    apool = torch.mean(conv_features, dim = 1)
    mpool, _ = torch.max(conv_features, dim = 1)

    imgs_embed = torch.cat([apool, mpool], dim = 1).unsqueeze(1)

    words_embed = self.__content_embed__(input_ids)
    indices  = torch.arange(self.seq_len + 1).expand(batch, -1).to(device)
    position_embed = self.__position_embed__(indices)

    h = self.__embed_drop__(torch.cat([imgs_embed, words_embed], dim = 1) + position_embed).to(device) # (N, seq_len + 1, embed_dim)
    for i in range(self.layer_num):
        h = self.__hidden_layers__[i](h)[0]        
        h[:, :1, :] = imgs_embed + position_embed[:, :1, :]

    preds = self.__fc_layer__(self.dropout(self.__layer_norm__(h[:, 1:, :]))) # (N, seg_len, vocab_dim)
    return preds
  ...
  def __sample__(self, images):
    ...
    indices = torch.arange(1).expand(batch, -1).to(device).long()
    position_embed = self.__position_embed__(indices)

    h = (torch.cat([imgs_embed, start_embed.unsqueeze(1)], dim = 1) + position_embed).to(device) # (N, 2, embed_dim)
    ...
    for i in range(self.seq_len):
      ...
      pred = self.__fc_layer__(self.__layer_norm__(h[:, 1:, :]))[:, -1, :] # (N, vocab_dim)
      preds[:, i] = self.__random_topk__(pred = pred, k = 1) 
      scores[:, i, :] = pred
      
      words_embed = self.__content_embed__(preds[:, :(i + 1)].long())
      indices = torch.arange(i + 3).expand(batch, -1).to(device)
      position_embed = self.__position_embed__(indices)
