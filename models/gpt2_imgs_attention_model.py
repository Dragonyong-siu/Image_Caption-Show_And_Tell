# model : encoder-resnet, decoder-transformer
...
class decoder(nn.Module):
  ...
  def forward(self, images, input_ids = None):
    ...
    image_blocks = self.__img_block__(images, self.block_num).reshape(-1, 3, self.block_size, self.block_size)
    batch_features = self.__encoder__(image_blocks).reshape(batch, int(self.block_num ** 2), self.features_dim) 
    imgs_embed = self.__img2embed_layer__(batch_features) # (N, block_num ** 2, embed_dim)

    words_embed = self.__content_embed__(input_ids[:, 1:])
    indices = torch.arange(self.seq_len + int(self.block_num ** 2) - 1).expand(batch, -1).to(device)
    ...
    preds = self.__fc_layer__(self.dropout(self.__layer_norm__(h))) # (N, seg_len + block_num ** 2 - 1, vocab_dim)
    return preds[:, (int(self.block_num ** 2) - 1):, :]
  ...
  def __sample__(self, images):
    ...
    h = imgs_embed + self.__position_embed__(torch.arange(4).expand(batch, -1)).to(device).long()) # (N, block_num ** 2, embed_dim)
    ...
    for i in range(self.seq_len):
      ...
      indices = torch.arange(i + 5).expand(batch, -1).to(device)
      position_embed = self.__position_embed__(indices)

      h = torch.cat([imgs_embed, words_embed], dim = 1) + position_embed

    return scores

gpt_decoder = decoder(features_dim = 512 * 3 * 3,
                      embed_dim = 768)
